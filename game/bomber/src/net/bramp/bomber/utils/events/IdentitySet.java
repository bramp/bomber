 ...
 * size.
 * <br>
 * This is a copy of ObjectSet slightly changed to support Identity.
 * @author Nathan Sweet
 * @author bramp
 */
public class IdentitySet<T> implements Iterable<T> {
	...
	private SetIterator<T> iterator1, iterator2;
	...
	public IdentitySet (int initialCapacity, float loadFactor) {
		...
		keyTable = newArray(capacity + stashCapacity);
	}
	...
	public void addAll (Array<T> array) {
		ensureCapacity(size + array.size);
		for (int i = 0, n = array.size; i < n; i++)
			add(array.get(i));
	}

	public void addAll (IdentitySet<T> set) {
		ensureCapacity(size + set.size);
		for (T key : set)
			add(key);
	}
	...
	/** Increases the size of the backing array to accommodate the specified number items. 
	 * Useful before adding many items to avoid multiple backing array resizes. */
	public void ensureCapacity (int minCapacity) {
		if (minCapacity >= threshold) resize(MathUtils.nextPowerOfTwo((int)(minCapacity / loadFactor)));
	}
	...
	private void resize (int newSize) {
		...
		keyTable = newArray(newSize + stashCapacity);
	...
	public SetIterator<T> iterator () {
		if (iterator1 == null) {
			iterator1 = new SetIterator<T>(this);
			iterator2 = new SetIterator<T>(this);
		}
		if (!iterator1.valid) {
			iterator1.reset();
	...
	static public class SetIterator<K> implements Iterable<K>, Iterator<K> {
		...
		public SetIterator (IdentitySet<K> set) {
			this.set = set;
			reset();
		}
		...
		public Array<K> toArray () {
			Array<K> array = new Array<K>(true, set.size);
			while (hasNext)
				array.add(next());
			return array;
		}
